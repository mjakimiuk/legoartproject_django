...
class LEGO_plate(object):
    ...
    def __init__(self,pdf, cord_x, cord_y, colour, text):
        self.pdf = pdf
        self.cord_x = cord_x
        self.cord_y = cord_y
        self.colour = colour
    ...
    def print_it(self):
        R, G, B = self.colour
        n = 2
        self.pdf.set_fill_color(r=R, g=G, b=B)
        self.pdf.ellipse(
            y=9 * n + self.cord_x,
            x=8.5 * n + self.cord_y,
            w=3.5 * n,
        ...
        self.pdf.text(14 * n + self.cord_y, 10.2 * n + self.cord_x, self.text)
